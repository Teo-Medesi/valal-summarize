...
const summarize = async (input, options = {language, length, temperature, custom}) => {
    ...
    if (!input || !options.language || !options.custom || !options.length || !options.temperature) throw new Error("Missing input or parameters!");

    const prompt = generatePrompt( options.input, {
